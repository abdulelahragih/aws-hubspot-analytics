...
export class HubspotAnalyticsStack extends cdk.Stack {
    constructor(scope: Construct, id: string, props?: cdk.StackProps) {
        ...
        const invokePipelinesDimFn = new stepfunctionstasks.LambdaInvoke(
            this,
            "InvokePipelinesDimFunction",
            {
                lambdaFunction: pipelinesDimFn,
                resultPath: "$.pipelinesDimResult",
            }
        );
        ...
        const startCuratedCrawler = new stepfunctionstasks.CallAwsService(this, "StartCuratedCrawler", {
            service: "glue",
            action: "startCrawler",
            parameters: {Name: crawler.name!},
            resultPath: "$.curatedCrawlerResult",
            iamResources: [`arn:aws:glue:${this.region}:${this.account}:crawler/${crawler.name}`],
        });

        const startDimCrawler = new stepfunctionstasks.CallAwsService(this, "StartDimCrawler", {
            service: "glue",
            action: "startCrawler",
            parameters: {Name: dimCrawler.name!},
            resultPath: "$.dimCrawlerResult",
            iamResources: [`arn:aws:glue:${this.region}:${this.account}:crawler/${dimCrawler.name}`],
        });


        // Now define the complete workflow with proper crawler references
        // Success notification
        const successNotification = new stepfunctionstasks.SnsPublish(this, "SendSuccessNotification", {
            topic: notificationTopic,
            subject: "HubSpot Data Ingest - Success",
            message: stepfunctions.TaskInput.fromJsonPathAt("$.message"),
        });

        const failureNotification = new stepfunctionstasks.SnsPublish(this, "SendFailureNotification", {
            topic: notificationTopic,
            subject: "HubSpot Data Ingest - Failure",
            message: stepfunctions.TaskInput.fromJsonPathAt("$.errorDetails.message"),
        });
